...
function extractFileNames(cwd, provider, functions) {
    ...
    return _.values(functions)
        ...
        throw new Error(`Typescript compilation failed. Please ensure handlers exists with ext .ts or .js ${JSON.stringify(cwd)}xx${JSON.stringify(h)}`);
    });
}
...
function getTypescriptConfig(cwd, logger) {
